...
function readCountdown() {
  const m = Math.max(0, Math.min(999, parseInt($("minutes").value || 0, 10)));
  const s = Math.max(0, Math.min(59, parseInt($("seconds").value || 0, 10)));
  totalSeconds = m * 60 + s;
...
function pauseLoop() {
  ...
  $("startBtn").disabled = false;
  $("pauseBtn").disabled = true;
  setStatus(mode === "alarm" ? "ALARM PAUSED" : "SYSTEM PAUSED");
  log("PROCESS PAUSED");
  play(audio.key1);
}
...
function completeCountdown() {
  ...
  // The dramatic joke — harmless, entirely local, no actual destructive action.
  setTimeout(() => {
...
function switchMode(newMode) {
  ...
  $("countdownControls").classList.toggle("hidden", mode !== "countdown");
  $("timerControls").classList.toggle("hidden", mode !== "timer");
  $("alarmControls").classList.toggle("hidden", mode !== "alarm");
...
$("shutdownBtn").addEventListener("click", () => {
  ...
  $("boot-status"); // harmless no-op
  document.querySelector(".boot-status").textContent = "SYSTEM STANDBY";
});
...
$("minutes").addEventListener("input", () => {
  if (!running && mode === "countdown") readCountdown();
  play(audio.key1);
});
...
document.querySelectorAll(".presets button").forEach(btn => {
  btn.addEventListener("click", () => {
    ...
    $("seconds").value = 0;
    if (mode !== "countdown") switchMode("countdown");
    readCountdown();
...
setInterval(() => {
  ...
  if (mode === "alarm" && running) checkAlarm();
}, 250);
