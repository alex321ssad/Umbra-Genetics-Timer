...
// Dedicated completion sound playback.
if (audio.complete) {
audio.complete.pause();
audio.complete.currentTime = 0;

```
const completePromise = audio.complete.play();

if (completePromise !== undefined) {
  completePromise.catch((err) => {
    console.warn("Complete sound failed:", err);
  });
}
```
...
// with no actual destructive action.
setTimeout(() => {
